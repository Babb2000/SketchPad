...
TakeAways:
---
1. The first problem I had when going about starting this project was what to start first. Recalling a previous article I read on web-dev, I recalled that a good 
way to start a project is first to create a design doc. Using google drive I broke down the project into smaller tasks that I could focus on. One of them
...
hard coded the result into html to wrap my mind around how the grid should look. Then I began to use javascript's DOM manipulation to retrieve elements and the `document
.createElement('div')` to dynamically add div's into the container div. 
3. When looping through the number of columns I wanted to add to the container, and I realized that I also had to nest another for-loop within the current loop to add the
...
4. The final portion of the project that I spent a little more time on was adding the "hover" effect to the grid layout. I chose to do this by using a javascript event handler
